...
/// @dev ERC20-numeraire handler for multi-entry-per-market PredictionMigrator invariants.
contract PredictionMigratorMultiEntryInvariantHandler is Test {
    ...
    function _claimWinner(
        ...
        uint256 actorNumeraireBefore = numeraire.balanceOf(actor);
        vm.startPrank(actor);
        winningToken.approve(address(migrator), claimTokenAmount);
        migrator.claim(oracle, claimTokenAmount);
        vm.stopPrank();

        uint256 actorNumeraireDelta = numeraire.balanceOf(actor) - actorNumeraireBefore;
        assertEq(actorNumeraireDelta, expectedPayout, "claim payout mismatch");
