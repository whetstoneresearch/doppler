...
contract SwapRestrictorMigratorInvariantTests is Deployers {
    ...
    SwapRestrictorMigratorHandler public handler;
    ...
    function setUp() public {
        ...
        hooks[0] = address(swapRestrictorHook);
        uint256[] memory flags = new uint256[](1);
        flags[0] = ON_INITIALIZATION_FLAG | ON_SWAP_FLAG;
        vm.prank(AIRLOCK_OWNER);
        migrator.setDopplerHookState(hooks, flags);

        handler.setMigrator(migrator, swapRestrictorHook, locker);

        bytes4[] memory selectors = new bytes4[](3);
        selectors[0] = handler.initialize.selector;
        selectors[1] = handler.swapApproved.selector;
        selectors[2] = handler.swapUnapproved.selector;

        targetSelector(FuzzSelector({ addr: address(handler), selectors: selectors }));
        targetContract(address(handler));
    }

    function invariant_AmountLeftNeverExceedsMax() public view {
        ...
        for (uint256 i; i < poolKeysLength; i++) {
            ...
            uint256 maxAmount = handler.getMaxAmount(poolId);
            uint256 remaining = handler.getAmountLeft(poolId);

            assertLe(remaining, maxAmount, "Remaining amount exceeds max");
        }
    }
}

struct SwapRestrictorSettings {
    address asset;
    address numeraire;
    bool isToken0;
}

contract SwapRestrictorMigratorHandler is Test {
    using LibAddressSet for AddressSet;

    IPoolManager public manager;
    DopplerHookMigrator public migrator;
    SwapRestrictorDopplerHook public hook;
    StreamableFeesLockerV2 public locker;
    PoolSwapTest public swapRouter;

    mapping(PoolId => SwapRestrictorSettings) public settingsOf;
    PoolKey[] public poolKeys;
    uint256 public poolKeysLength;
    address[] public availableNumeraires;

    mapping(PoolId => uint256) public maxAmountOf;
    mapping(PoolId => uint256) public lastRemainingOf;

    AddressSet internal actors;
    address internal currentActor;

    address internal approvedBuyer = address(0xBEEF);
    ...
    function setMigrator(
        ...
        SwapRestrictorDopplerHook hook_,
    ...
    function initialize(uint256 seed) public {
        ...
        lastRemainingOf[poolKey.toId()] = hook.amountLeftOf(poolKey.toId(), approvedBuyer);
    }

    function swapApproved(uint256 seed) public {
        if (poolKeys.length == 0) return;

        uint256 swapAmount = 0.1 ether;
        PoolKey memory poolKey = poolKeys[seed % poolKeys.length];
        PoolId poolId = poolKey.toId();
        SwapRestrictorSettings memory settings = settingsOf[poolId];

        if (settings.numeraire == address(0)) {
            deal(approvedBuyer, swapAmount);
        } else {
            deal(settings.numeraire, approvedBuyer, swapAmount);
            vm.prank(approvedBuyer);
        ...
        lastRemainingOf[poolId] = hook.amountLeftOf(poolId, approvedBuyer);
    }

    function swapUnapproved(uint256 seed) public createActor {
        if (poolKeys.length == 0) return;

        uint256 swapAmount = 0.1 ether;
        PoolKey memory poolKey = poolKeys[seed % poolKeys.length];
        SwapRestrictorSettings memory settings = settingsOf[poolKey.toId()];
    ...
    function getMaxAmount(PoolId poolId) external view returns (uint256) {
        return maxAmountOf[poolId];
    }

    function getAmountLeft(PoolId poolId) external view returns (uint256) {
        return hook.amountLeftOf(poolId, approvedBuyer);
    }
}
