...
contract ForkTest is BaseForkTest {
    ...
    function testNoOpVsFullGovernance() public {
        ...
        CreateParams memory noOpParams = CreateParams({
            ...
            poolInitializerData: abi.encode(
                InitData({ fee: 3000, tickLower: 46_080, tickUpper: 92_160, numPositions: 1, maxShareToBeSold: 1e17 })
            ),
    ...
    function testModuleDeployments() public {
        ...
        assertEq(
            uint256(airlock.getModuleState(address(noOpGovernanceFactory))), uint256(ModuleState.GovernanceFactory)
        );
    ...
    function testV3WithNoOpGovernance() public {
        ...
        int24 tickLower = 46_080; // ~1.5x price
        int24 tickUpper = 92_160; // ~100x price

        bytes memory tokenData = _encodeTokenData("Simple Token", "SIMPLE");
        bytes memory v3InitData = abi.encode(
            InitData({
                fee: 3000,
                tickLower: tickLower,
                tickUpper: tickUpper,
    ...
    function testV4WithNoOpGovernance() public {
        ...
        bytes memory v4InitData = abi.encode(
            ...
            int24(-200_000), // startingTick
            int24(200_000), // endingTick
