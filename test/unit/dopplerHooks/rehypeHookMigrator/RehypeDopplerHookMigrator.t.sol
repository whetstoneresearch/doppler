...
contract RehypeDopplerHookMigratorTest is Test {
    ...
    function setUp() public {
        ...
        rehypeHookMigrator =
            new RehypeDopplerHookMigrator(DopplerHookMigrator(payable(address(mockMigrator))), poolManager);
    }
    ...
    function test_onInitialization_InitializesPosition(PoolKey memory poolKey) public {
        ...
        bytes memory data = abi.encode(_quarterMigratorInitData(numeraire, buybackDst, 3000, FeeRoutingMode.DirectBuyback));
    ...
    function test_onInitialization_RevertsWhenSenderNotMigrator(PoolKey memory poolKey) public {
        bytes memory data = abi.encode(_quarterMigratorInitData(address(0), address(0), 0, FeeRoutingMode.DirectBuyback));
    ...
    function test_onInitialization_SetsFeeRoutingModeFromCalldata(PoolKey memory poolKey) public {
        ...
        bytes memory data =
            abi.encode(_quarterMigratorInitData(numeraire, buybackDst, 3000, FeeRoutingMode.RouteToBeneficiaryFees));
    ...
    function test_onInitialization_RevertsWhenFeeRoutingModeInvalid(PoolKey memory poolKey) public {
        ...
        // Manually encode with MigratorInitData field order but invalid feeRoutingMode (2 is out of enum range)
        bytes memory data = abi.encode(
            numeraire,
            buybackDst,
            uint24(3000),
    ...
    function test_setFeeDistribution_UpdatesDistribution(PoolKey memory poolKey) public {
        ...
        bytes memory data = abi.encode(_quarterMigratorInitData(numeraire, buybackDst, 0, FeeRoutingMode.DirectBuyback));
    ...
    function test_setFeeDistribution_RevertsWhenDoesNotAddToWAD(PoolKey memory poolKey) public {
        ...
        vm.prank(buybackDst);
        vm.expectRevert(FeeDistributionMustAddUpToWAD.selector);
        rehypeHookMigrator.setFeeDistribution(poolKey.toId(), 0.5e18, 0.5e18, 0.5e18, 0, 0.5e18, 0.5e18, 0.5e18, 0);
    }
    ...
    function test_setFeeDistribution_AllToBuybacks(PoolKey memory poolKey) public {
        ...
        vm.prank(buybackDst);
        rehypeHookMigrator.setFeeDistribution(poolId, 0.5e18, 0.5e18, 0, 0, 0.5e18, 0.5e18, 0, 0);

        (uint256 a, uint256 n, uint256 b, uint256 l, uint256 a2, uint256 n2, uint256 b2, uint256 l2) =
            rehypeHookMigrator.getFeeDistributionInfo(poolId);
        assertEq(a, 0.5e18);
        assertEq(n, 0.5e18);
        assertEq(b, 0);
    ...
    function test_setFeeDistribution_CanBeCalledMultipleTimes(PoolKey memory poolKey) public {
        ...
        // Second update
        vm.prank(buybackDst);
        rehypeHookMigrator.setFeeDistribution(poolId, 0, 0, 0, WAD, 0, 0, 0, WAD);
    ...
    function test_collectFees_ReturnsZeroWhenNoFees(PoolKey memory poolKey) public {
        ...
        bytes memory data = abi.encode(_quarterMigratorInitData(numeraire, address(0), 0, FeeRoutingMode.DirectBuyback));
    ...
    function test_onInitialization_PositionTicksMatchTickSpacing(int24 tickSpacing) public {
        ...
        tickSpacing = int24(bound(int256(tickSpacing), 1, 16_383));
