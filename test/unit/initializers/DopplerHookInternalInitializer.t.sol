...
contract MockCustomAccountingDopplerHook is IDopplerHook {
    function onInitialization(address, PoolKey calldata, bytes calldata) external { }
    function onSwap(
        address,
        PoolKey calldata,
...
contract MockNegativeCustomAccountingDopplerHook is IDopplerHook {
    function onInitialization(address, PoolKey calldata, bytes calldata) external { }
    function onSwap(
        address,
        PoolKey calldata,
...
contract DopplerHookMulticurveInternalInitializerTest is Deployers {
    ...
    function test_getHookPermissions_DisablesAfterSwapReturnDelta() public view {
        Hooks.Permissions memory permissions = initializer.getHookPermissions();
    ...
    function test_afterSwap_PassesWhenMsgSenderPoolManager(
        ...
        vm.prank(address(manager));
        (bytes4 selector, int128 delta) = initializer.afterSwap(address(this), key, params, balanceDelta, hookData);
        assertEq(selector, initializer.afterSwap.selector);
        assertEq(delta, 0);
    }
    ...
    function test_afterSwap_RevertsWhenDopplerHookReturnsNonZeroDelta(InitDataParams memory initParams, bool isToken0)
        public
        prepareAsset(isToken0)
    {
        MockCustomAccountingDopplerHook customAccountingHook = new MockCustomAccountingDopplerHook();
    ...
    function test_afterSwap_RevertsWhenDopplerHookReturnsNegativeDelta(
        InitDataParams memory initParams,
        bool isToken0
    ) public prepareAsset(isToken0) {
        MockNegativeCustomAccountingDopplerHook customAccountingHook = new MockNegativeCustomAccountingDopplerHook();
