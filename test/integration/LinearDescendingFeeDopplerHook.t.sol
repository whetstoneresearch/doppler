...
contract LinearDescendingFeeDopplerHookIntegrationTest is Deployers {
    ...
    LinearDescendingFeeDopplerHook public descendingFeeHook;
    ...
    function setUp() public {
        ...
        descendingFeeHook = new LinearDescendingFeeDopplerHook(address(initializer));
        ...
        dopplerHooks[0] = address(descendingFeeHook);
        flags[0] = ON_INITIALIZATION_FLAG | ON_SWAP_FLAG;
        initializer.setDopplerHookState(dopplerHooks, flags);
        vm.stopPrank();
    }

    function test_swap_DescendsFeeLinearly() public {
        uint24 startFee = 20_000;
        uint24 endFee = 5_000;
        uint64 durationBlocks = 10;

        (bool isToken0,) = _createToken(bytes32(uint256(1)), startFee, endFee, durationBlocks);

        (,,, uint24 lpFeeBefore) = manager.getSlot0(poolId);
        assertEq(lpFeeBefore, startFee, "initial dynamic fee mismatch");

        (,,,, uint64 startBlock,,) = descendingFeeHook.getFeeScheduleOf(poolId);

        vm.roll(startBlock + 1);
        _buyAsset(isToken0);
        (,,, uint24 lpFeeAfterBlock1) = manager.getSlot0(poolId);
        assertEq(lpFeeAfterBlock1, 18_500, "fee should descend linearly at block +1");

        vm.roll(startBlock + durationBlocks);
        _buyAsset(isToken0);
        (,,, uint24 lpFeeAtEnd) = manager.getSlot0(poolId);
        assertEq(lpFeeAtEnd, endFee, "fee should reach terminal value at duration");

        vm.roll(startBlock + durationBlocks + 5);
        _buyAsset(isToken0);
        (,,, uint24 lpFeeAfterEnd) = manager.getSlot0(poolId);
        assertEq(lpFeeAfterEnd, endFee, "fee must remain terminal after schedule completes");
    }
    ...
    function _prepareInitData(address token, uint24 startFee, uint24 endFee, uint64 durationBlocks)
        internal
        returns (InitData memory)
    {
        ...
        bytes memory hookData =
            abi.encode(FeeScheduleParams({ startFee: startFee, endFee: endFee, durationBlocks: durationBlocks }));
        ...
        return InitData({
            ...
            dopplerHook: address(descendingFeeHook),
            onInitializationDopplerHookCalldata: hookData,
            graduationDopplerHookCalldata: new bytes(0)
        });
    }

    function _createToken(bytes32 salt, uint24 startFee, uint24 endFee, uint64 durationBlocks)
        internal
        returns (bool isToken0, address asset)
    {
        ...
        InitData memory initData = _prepareInitData(tokenAddress, startFee, endFee, durationBlocks);
