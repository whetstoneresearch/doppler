...
Automatic verification fails because deployments compile `DopplerHookInitializer` as part of the full project compiler batch, while standard verification recompiles only its transitive dependencies. With `via_ir = true`, the different source-unit and AST numbering changes the generated bytecode even when the source and compiler settings are otherwise identical.

Use the verification script from the deployment commit with its matching `out`, `cache`, and `out/build-info` artifacts. Set `ETHERSCAN_API_KEY`, then run:

```sh
bun scripts/VerifyDopplerHookInitializer.ts <chain-id> --dry-run
bun scripts/VerifyDopplerHookInitializer.ts <chain-id>
```

The dry run confirms an exact creation-bytecode match. The second command reconstructs the original compiler layout, reads the deployment address and constructor arguments from `deployments.config.toml`, and submits the verification through Etherscan V2.
