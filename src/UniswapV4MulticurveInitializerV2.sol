...
import { FeesManagerV2 } from "src/base/FeesManagerV2.sol";
 ...
 */
contract UniswapV4MulticurveInitializerV2 is IPoolInitializer, FeesManagerV2, ImmutableAirlock, MiniV4Manager {
        ...
        )
    {
        ...
        Curve[] memory curves = state.curves;
        int24 tickSpacing = state.poolKey.tickSpacing;
        uint256 totalTokensOnBondingCurve = state.totalTokensOnBondingCurve;

        Position[] memory positions = _computePositions(curves, tickSpacing, totalTokensOnBondingCurve, isToken0);
    ...
    /// @inheritdoc FeesManagerV2
    function _collectFees(PoolId poolId, bytes calldata data) internal override returns (BalanceDelta fees) {
        address asset = getAsset[poolId];
        PoolState memory state = getState[asset];
        require(state.status == PoolStatus.Locked, PoolNotLocked());
        // bool isToken0 = asset == Currency.unwrap(state.poolKey.currency0);
        // (Curve[] memory adjustedCurves,,) = adjustCurves(state.curves, 0, state.poolKey.tickSpacing, isToken0);
        // calculatePositions(adjustedCurves, state.poolKey.tickSpacing, state.totalTokensOnBondingCurve, 0, isToken0);
        Position[] memory positions = abi.decode(data, (Position[]));
    ...
    function _computePositions(
        ...
        (Curve[] memory adjustedCurves,,) = adjustCurves(curves, 0, tickSpacing, isToken0);

        return calculatePositions(adjustedCurves, tickSpacing, totalTokensOnBondingCurve, 0, isToken0);
    }
}
