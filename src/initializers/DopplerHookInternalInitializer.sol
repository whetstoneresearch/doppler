...
/// @notice Thrown when a Doppler hook attempts to return non-zero swap delta
error CustomAccountingNotSupported(address dopplerHook, int128 hookDelta);
...
/**
 * @title Doppler Hook Internal Initializer
 ...
 */
contract DopplerHookInternalInitializer is ImmutableAirlock, BaseHook, MiniV4Manager, FeesManager, IPoolInitializer {
    ...
    function _afterSwap(
        ...
        if (dopplerHook != address(0) && isDopplerHookEnabled[dopplerHook] & ON_SWAP_FLAG != 0) {
            (, int128 delta) = IDopplerHook(dopplerHook).onSwap(sender, key, params, balanceDelta, data);
            if (delta != 0) {
                revert CustomAccountingNotSupported(dopplerHook, delta);
            }
        }
