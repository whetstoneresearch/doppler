...
import { MineDopplerHookInternalInitializerParams, mineDopplerHookInternalInitializer } from "test/shared/AirlockMiner.sol";

contract DeployDopplerHookInternalInitializerScript is Script, Config {
    function run() public {
    ...
    function deployToChain(uint256 chainId) internal {
        ...
        (bytes32 salt, address deployedTo) =
            mineDopplerHookInternalInitializer(MineDopplerHookInternalInitializerParams({ sender: msg.sender, deployer: createX }));
        address dopplerHookInitializer = ICreateX(createX)
            .deployCreate3(
                salt, abi.encodePacked(type(DopplerHookInternalInitializer).creationCode, abi.encode(airlock, poolManager))
            );
        require(dopplerHookInitializer == deployedTo, "Unexpected deployed address");
        vm.stopBroadcast();
        config.set("doppler_hook_internal_initializer", dopplerHookInitializer);
    }
}
